...
class Digestor:
    ...
    def __init__(self, school, meta, tracker=NullTracker()):
        ...
        self.cache = DotDict(dict(course={"code": None}, section={"code": None}))

        self.school = school
        self.adapter = DigestionAdapter(
            school, self.cache, SCHOOLS_MAP[self.school].short_course_weeks_limit
        )
        self.meta = meta
    ...
    def digest_meeting(self, meeting, section_model=None):
        ...
        offering_models = []
        for offering in self.adapter.adapt_meeting(
            meeting, section_model=section_model
        ):
...
class DigestionAdapter:
    ...
    def adapt_course(self, course):
        ...
        for same_as in course.get("same_as", []):
            same_as = Course.objects.filter(school=self.school, code=same_as).first()
            if same_as is not None:
                adapted["same_as"] = same_as
                # TODO - create double-sided relation

        return {"code": course.code, "school": self.school, "defaults": adapted}
    ...
    def adapt_section(self, section, course_model=None):
        ...
        if course_model is None:
            if self.cache.course and section.course.code == self.cache.course.code:
                course_model = self.cache.course
            else:
                course_model = Course.objects.filter(
                    school=self.school, code=section.course.code
                ).first()
                if course_model is None:
                    # TODO - run tests with different database
                    print(
                        "course %s section not already in database".format(
                            section.course.code
                        ),
                        file=sys.stderr,
                    )
        ...
        semester, _ = Semester.objects.update_or_create(
            name=section.term, year=section.year
        )
        if semester is None:
            raise DigestionError(
                "Semester {} {} not in DB".format(section.term, section.year)
            )
    ...
    def adapt_meeting(self, meeting, section_model=None):
        ...
        if section_model is None:
            ...
            if self.cache.code and meeting.course.code == self.cache.course.code:
                ...
                course_model = Course.objects.filter(
                    school=self.school, code=meeting.course.code
                ).first()
                if course_model is None:
                    print(
            ...
            if (
                ...
                and meeting.section.code == self.cache.section.meeting_section
            ):
        ...
        for day in meeting.get("days", []):
            offering = {
                ...
                "defaults": {
                    "location": meeting.get("location", {}).get("building", "")
                    + " "
...
class Vommit(DigestionStrategy):
    ...
    def diff(self, kind, inmodel, dbmodel, hide_defaults=True):
        ...
        for k, v in inmodel.items():
            ...
            try:
                whats[k] = str(v)
            except (django.utils.encoding.DjangoUnicodeDecodeError, UnicodeEncodeError):
...
class Absorb(DigestionStrategy):
    ...
    def remove_section(section_code, course_obj):
        ...
        if Section.objects.filter(
            course=course_obj, meeting_section=section_code
        ).exists():
            s = Section.objects.get(course=course_obj, meeting_section=section_code)
            s.delete()
