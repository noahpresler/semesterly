...
def parse_results(source):
    ...
    for section_header in section_headers:
        ...
        if (
            "not been informed" in sibling.text.lower()
        ):  # filter out the textbook-less sections
            continue
        matches = re.search("- (.+)[YFS], section (.+?) ", section_header.text)
        all_textbooks_info = sibling.find_all("td", class_="book-desc")
        print(
            "\t\t\tFor {} section {}, found {} textbook(s). These are:".format(
        ...
        course_sections = Section.objects.filter(
            course=course, meeting_section=matches.group(2)
        )
        for textbook_info in all_textbooks_info:
            ...
            textbook, created = Textbook.objects.update_or_create(
                isbn=isbn, defaults=textbook_data
            )
...
def process_campus(campus_info, semester="F"):
    ...
    rest = "control=campus&campus=%s&term=%s" % (
    ...
    campus_courses = Course.objects.filter(
        school="uoft", code__endswith=campus_info["ending"]
    ).order_by("code")
    current_dept = ""
    for i, course in enumerate(campus_courses):
        ...
        if course.department != current_dept:
            ...
            rest = "control=department&dept=%s&term=%s" % (
                department_to_dept_id[current_dept],
                campus_info["term"],
            )
            response = get_request(rest)
            ...
            for soup_course in soup_courses:
                ...
                if code[-1].upper() not in [semester, "Y"]:
                    print(
                        "Skipping",
                        code,
                        "because this entry was for different semester",
                    )
                    continue
        ...
        rest = "control=course&course=%s&term=%s" % (
            course_to_course_id[course.code],
            campus_info["term"],
        )
        response = get_request(rest)
        ...
        for section in available_sections:
            ...
            rest = "control=section&section=%s&t=%s" % (
                section_to_section_id[section],
                int(time.time()),
            )
            response = get_request(rest)
