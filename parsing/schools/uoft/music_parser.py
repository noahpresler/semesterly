...
class UofTMusicWriter(BaseWriter):
    def __init__(self, sheet_path="scripts/uoft/timetable2016-17.xls"):
        self.data = get_tt_df(
            sheet_path
        )  # get data first to pass num rows to base parser
        BaseWriter.__init__(self)
        self.row_iter = self.data.iterrows()
        self.next_course_row = next(self.row_iter)[
            1
        ]  # stores the header row of the next course

    def get_course_elements(self):
        try:  # iterate through iterator until the end
            while True:
                course_header_row = self.next_course_row
                while not course_header_row.sem or not re.match(
                    "[A-Z]{3}[0-9]{3}[HY][0-9]", course_header_row.course_code
                ):
    ...
    def parse_section_element(self, section_element):
        ...
        section_data = {
            ...
            "instructors": ", ".join(
                [row.instructor for row in section_element if row.instructor]
            ),
        }
        ...
        for meeting in meetings:
            ...
            if split_index > 1:  # more than one day
                for i in range(1, split_index):
                    ...
                    copy[0].day_time = (
                        meeting[0].day_time[i] + meeting[0].day_time[split_index:]
                    )
                    meetings.append(copy)
                meeting[0].day_time = (
                    meeting[0].day_time[0] + meeting[0].day_time[split_index:]
                )
    ...
    def parse_meeting_element(self, meeting_element):
        ...
        meeting_data = {
            ...
            # optional
            "location": ", ".join(
                [str(row.location) for row in meeting_element if row.location]
            ),
        }
        return meeting_data
...
def get_tt_df(path):
    ...
    return xl.parse(
        "FM", skiprows=4, parse_cols=len(colnames) - 1, names=colnames
    ).fillna("")
