...
class Parser(BaseParser):
    ...
    def start_engineering(self):
        ...
        try:
            for semester in ["fall", "winter"]:
                ...
                request_url = "http://www.apsc.utoronto.ca/timetable/{}.html".format(
                    semester
                )
                soup = BeautifulSoup(
                    self.s.get(url=request_url, cookies=self.cookies, timeout=15).text
                )
                ...
                for table in tables:
                    ...
                    for row in rows:
                        ...
                        if day not in engineering_day_map:
                            continue
                        course = course.strip()
                ...
                for course in json:
                    ...
                    for section in json[course]:
                        ...
                        for offering_dict in json[course][section]["offerings"]:
                            o, created = Offering.objects.update_or_create(
                                section=S,
                                day=offering_dict["day"],
                                time_start=offering_dict["time_start"],
                                time_end=offering_dict["time_end"],
                                defaults={"location": offering_dict["location"]},
                            )
                            o.save()

            print(
                "Done Engineering, found %d new courses (collectively) so far. Now wrapping up..."
                % (self.new)
            )
        except requests.ConnectionError:
            print(
                "Couldn't connect. Found %d new courses (collectively) so far. Now wrapping up..."
                % (self.new)
            )
        self.wrap_up()
    ...
    def start(self, **kwargs):
        ...
        for year_of_study in self.years_of_study:
            ...
            request_url = "https://timetable.iit.artsci.utoronto.ca/api/courses?org=&code=&section=&studyyear={}&daytime=&weekday=&prof=&breadth=".format(
                year_of_study
            )
            ...
            for key in data:
                try:
                    ...
                    for section_key in meetings:
                        section = (
                            section_key.split("-")[0][0] + section_key.split("-")[-1]
                        )
                        ...
                        size = (
                            section_data["enrollmentCapacity"]
                            if section_data["enrollmentCapacity"]
                            else 0
                        )
                        ...
                        for offering in schedule:
                            ...
                            try:
                                CO, co_created = Offering.objects.update_or_create(
                                    section=S,
    ...
    def start_utm(self):
        ...
        for year_of_study in self.years_of_study:
            ...
            response = requests.post(
                "https://student.utm.utoronto.ca/timetable/formatCourses2.php",
                data=payload,
            ).text
            ...
            for course_div in course_divs:
                ...
                try:
                    desc_end_index = re.search(
                        r"\[\d.+\]", course_desc_and_excl
                    ).start()
                except:
                ...
                if excl_or_prereq:
                    if "exclusion" in excl_or_prereq.text.lower():
                        ...
                        try:
                            maybe_prereqs = (
                                exclusions.nextSibling.nextSibling.nextSibling
                            )
                ...
                for si in section_data:
                    section, instructors, enrolment, size, waitlist = (
                        si[2],
                        si[3],
                        si[4],
                        si[5],
                    ...
                    if si[10] == "":
                        locations = [""] * len(days)
                    if len(locations) == 2 * len(days) or code[:-2] in [
                        "FAS147",
                        "FAS247",
                        "FAS343",
                        "FAS447",
        ...
        print(
            "Done UTM, found %d new courses (collectively) so far. Now starting UTSC."
            % (self.new)
        )
        self.start_utsc()
    ...
    def get_excl_prereq_breadth(self, tag):
        ...
        while tag and tag.name != "a":
            ...
            while (
                tag.nextSibling
                and tag.nextSibling.name != "a"
                and ":" not in tag.nextSibling
            ):
                try:
    ...
    def get_course_details(self, code, link):
        try:
            ...
            try:
                if (
                    a.next_sibling.next_sibling.strip
                    or a.next_sibling.next_sibling.text.strip()
                ):
        ...
        while i < len(trs):
            ...
            if self.is_tr_new_course(tr):
                ...
                if section_info["meeting_section"]:
                    meeting_section = (
                        section_info["meeting_section"][0]
                        + section_info["meeting_section"][3:]
                    )

                if section_info["time_start"] == "" or section_info["time_end"] == "":
                    print(
                        "\tInvalid details for course",
                        code,
                        section_info["meeting_section"],
                        ". Perhaps online?\n",
                    )
                    i += 1
                    continue
                ...
                CO, co_created = Offering.objects.update_or_create(
                    ...
                    defaults={"location": section_info["location"]},
                )
                ...
                print(
                    "\t\t",
                    section_info["day"] + ":",
                    section_info["time_start"] + "-" + section_info["time_end"],
                    "at",
            ...
            i += 1
        print(
            "Done UTSC, found %d new courses (collectively) so far. Now starting Engineering."
            % (self.new)
        )
