...
class Parser(BaseParser):
    ...
    def start(
        ...
        years_and_terms = dict_filter_by_dict(
            self._parse_years_and_terms(), years_and_terms_filter
        )
        for year, terms in years_and_terms.items():
            ...
            for term_name, term_code in terms.items():
                ...
                self.requester.get(
                    Parser.URL + "/SelectTerm!updateSessions.action", parse=False
                )
                ...
                for dept_code, dept_name in departments.items():
                    ...
                    params.update({"searchCriteria.subjectAreaCodes": dept_code})
    ...
    def parse_course(self, soup):
        ...
        soup = self.requester.get(
            ...
            params={"classNumber": course_number, "termCode": search.group(2)},
        )
    ...
    def parse_labeled_table(self, soup):
        ...
        for label in labels:
            ...
            if len(siblings) != 0:
                ...
                values = [
                    l
                    for l in (line.strip() for line in siblings[0].text.splitlines())
                    if l
                ]
    ...
    def parse_meeting_times(self, soup):
        ...
        values = (
            soup.find("tr", class_="courseHeader")
            .find_next_siblings()[0]
            .find_all("td")
        )
        ...
        try:
            for label, value in zip(
                [x.text.strip() for x in labels], [x.text.strip() for x in values]
            ):
    ...
    def extract_notes(self, soup):
        notes = " ".join(
            [l for l in (p.strip() for p in soup.text.splitlines()) if l]
        ).strip()
