...
class Parser(BaseParser):
    ...
    def _login(self):
        ...
        # Security checkpoint
        sec_block = soup.find("input", attrs={"name": "VSASM_ASVBlock"})["value"]
        ...
        # Security checkpoint
        vsasm_block = soup.find("input", attrs={"name": "VSASM_BLOCK"})["value"]
    ...
    def _parse_eval_results(self, school, area, course):
        ...
        soup = self.requester.post(
            Parser.BASE_URL,
            data={"ViewSchool": school, "ViewArea": area, "ViewCourse": course},
        )
    ...
    def _parse_eval_score_page(self, url):
        ...
        term = {"FALL": "Fall", "SPR": "Spring", "SUM": "Summer"}[rsem.group(1)]
        year = rsem.group(2)
        ...
        questions = (
            body.find("table").find("table").find_all("td", valign="top", width="200")
        )
        ...
        for question in questions:
            ...
            for adj, i in zip(adjs, list(range(len(adjs)))):
                ...
                if label != "No response" and question.text.strip() == question_text:
    ...
    def _parse_list_of_courses(self, school, area):
        soup = self.requester.post(
            Parser.BASE_URL, data={"ViewSchool": school, "ViewArea": area}
        )
        courses = soup.find("select", attrs={"name": "ViewCourse"}).find_all("option")
        return [c["value"].strip() for c in courses if c["value"].strip()]

    def _parse_list_of_areas(self, school):
        soup = self.requester.post(Parser.BASE_URL, data={"ViewSchool": school})
        areas = soup.find("select", attrs={"name": "ViewArea"}).find_all("option")
        return [a["value"] for a in areas if a["value"]]

    def _parse_list_of_schools(self):
        soup = self.requester.get(Parser.BASE_URL)
        schools = soup.find("select", attrs={"name": "ViewSchool"}).find_all("option")
    ...
    def _create_review_item(self, code, prof, score, summary, year, term):
        ...
        self.ingestor["instr"] = {
            "name": {"first": prof.split(",")[1], "last": prof.split(",")[0]}
        }
