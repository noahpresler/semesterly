...
class Parser(BaseParser):
    ...
    def start(self, **kwargs):
        ...
        Args:
            **kwargs: Nothing.
        """
        directory = "{}/{}/schools/jhu/HopkinsEvaluations".format(
    ...
    def _process_soup(self, soup, term, year):
        ...
        for cc in course_codes:
            ...
            score = self._get_score(profs.find_next(text=Parser.SCORE_PATTERN))
            summary = self._get_summary(
                title.find_next("p", text=Parser.SUMMARY_PATTERN)
            )
        ...
        while tag is not None:
            if tag.name == "p":
                if tag.text.find("write-in") != -1:
                    break
                elif "left:450px;" not in tag["style"] and len(tag.find_all()) == 0:
                    summary.append(tag.text)
                elif "left:108px" in tag["style"] and re.match(
                    Parser.CODE_PATTERN, tag.text
                ):
                    break
            tag = tag.find_next()
        return "".join(summary)
