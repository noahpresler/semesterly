...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        school_dir_path = "{}/{}/schools/{}".format(
            os.getcwd(), settings.PARSING_MODULE, code
        )
        ...
        with open(Command.template.format("parser"), "rb") as file:
            parser = file.read().format(
                name=name, code=code, parser_type="course".title()
            )

        with open(Command.template.format("config"), "rb") as file:
            config = file.read()
            for tag, replacement in replacements:
                config = config.replace("<{}>".format(tag), json.dumps(replacement))
