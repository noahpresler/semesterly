...
class BkstrDotComParser(BaseParser):
    ...
    def start(self, verbosity=3, departments_filter=None, years_and_terms_filter=None):
        """Start parsing."""
        ...
        for program, program_code in list(programs.items()):
            self._parse_program(program, program_code, query, years_and_terms_filter)

    def _parse_program(self, program, program_code, query, years_and_terms_filter):
        ...
        years_and_terms = dict_filter_by_dict(years_and_terms, years_and_terms_filter)
    ...
    def _parse_section(self, section, section_code, query):
        ...
        soup = self.requester.get(
            "{}/CourseMaterialsResultsView".format(BkstrDotComParser.URL), query2
        )
    ...
    def _extract_json(self, query):
        ...
        raw_text = self.requester.get(
            "{}/LocateCourseMaterialsServlet".format(BkstrDotComParser.URL),
            query,
            parse=False,
        ).text

        return json.loads(re.search(r"\'(.*)\'", raw_text).group(1))["data"][0]
