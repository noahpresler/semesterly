...
class PeoplesoftParser(BaseParser, metaclass=ABCMeta):
    ...
    AJAX_PARAMS = {"ICAJAX": "1", "ICNAVTYPEDROPDOWN": "0"}
    ...
    def parse(
        ...
        years_and_terms = dict_filter_by_dict(
            self._get_years_and_terms(soup), years_and_terms_filter
        )
            ...
            for term_name, term_code in terms.items():
                ...
                for group_id, group_name in list(groups.items()):
                    ...
                    if group_id is not None:
                        ...
                        params2 = PeoplesoftParser._hidden_params(soup, ajax=True)
                    ...
                    for dept_code, dept_name in departments.items():
                        ...
                        course_soups = self._get_course_list_as_soup(courses, soup)
    ...
    def _get_years_and_terms(self, soup):
        term_datas = soup.find("select", id="CLASS_SRCH_WRK2_STRM$35$").find_all(
            "option"
        )
        years_terms_values = {}
    ...
    def _get_dept_param_key(self, soup):
        return soup.find("select", id=re.compile(r"SSR_CLSRCH_WRK_SUBJECT_SRCH\$\d"))[
            "id"
        ]

    def _get_departments(self, soup, departments_filter=None):
        dept_soups = soup.find(
            "select", id=re.compile(r"SSR_CLSRCH_WRK_SUBJECT_SRCH\$\d")
        ).find_all("option")[1:]

        def extract_dept_name(d):
            return self.department_name_regex.match(d).group(1)

        departments = {d["value"]: extract_dept_name(d.text) for d in dept_soups}
    ...
    def _parse_course_description(self, soup):
        ...
        subtitle = soup.find("span", id="DERIVED_CLSRCH_SSS_PAGE_KEYDESCR").text
        ...
        components = soup.find(
            "div", id=re.compile(r"win\ddivSSR_CLS_DTL_WRK_SSR_COMPONENT_LONG")
        )
        ...
        if components is not None:
            ...
            if (
                len(components) > 1
                and self.ingestor["credits"] == 0
                and "Lecture" in components
                and "Lecture" != self.ingestor["section_type"]
                and self.ingestor["course_code"] in self.ingestor.validator.seen
            ):
                create_course = False
        ...
        for sched, loc, date in zip(scheds, locs, dates):
            ...
            if rsched:
                days = [
                    PeoplesoftParser.DAY_MAP[d]
                    for d in re.findall(r"[A-Z][^A-Z]*", rsched.group(1))
                ]
                time = (rsched.group(2), rsched.group(3))
    ...
    def _is_special_search(self, soup):
        return soup.find("span", class_="SSSMSGINFOTEXT") or soup.find(
            "span", id="DERIVED_SSE_DSP_SSR_MSG_TEXT"
        )
...
class UPeoplesoftParser(PeoplesoftParser):
    ...
    def _get_departments(self, soup, departments_filter=None):
        ...
        departments = soup.find_all(
            "a", id=re.compile(r"CLASS_SRCH_WRK2_SSR_PB_CLASS_SRCH\$\d")
        )
        department_names = soup.find_all(
            "span", id=re.compile(r"M_SR_SS_SUBJECT_DESCR\$\d")
        )
        depts = {
