...
def is_waitlist_only(course, semester):
    return any(
        sections_are_filled(sections)
        for sections in get_sections_by_section_type(course, semester).values()
    )
...
def get_sections_by_section_type(course, semester):
    ...
    return {
        section_type: list(grouped)
        for (section_type, grouped) in itertools.groupby(
            sections, attrgetter("section_type")
        )
    }
