...
def construct_preference_tt():
    ...
    if NO_CLASSES_BEFORE > 0 or NO_CLASSES_AFTER < 14 * 60 / get_granularity(SCHOOL):
        tt.append(
            [
                lambda co: not (
                    get_time_index_from_string(co[0].time_start) > NO_CLASSES_BEFORE
    ...
    if LEAST_DAYS:
        ...
        tt.append([(lambda co: co[0].day == "M"), (lambda co: co[0].day == "F")])

    # break time preference
    if BREAK_TIMES:
        break_periods = [
            BREAK_TIMES[i : i + BREAK_LENGTH]
            for i in range(len(BREAK_TIMES) - BREAK_LENGTH + 1)
        ]
        break_possibilities = [
            (
                lambda co: not (
                    get_time_index_from_string(co[0].time_start) > periods[-1]
...
def get_time_index(hours, minutes):
    ...
    return (hours - 8) * (60 / get_granularity(SCHOOL)) + minutes / get_granularity(
        SCHOOL
    )
