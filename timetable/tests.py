...
class Serializers(TestCase):
    def setUp(self):
        ...
        self.event = PersonalEvent.objects.create(
            name="gym", day="T", time_start="7:00", time_end="8:30"
        )
    ...
    def test_shared_timetable_serialization(self):
        timetable = SharedTimetable.objects.create(
            semester=self.sem, school="uoft", has_conflict=False
        )
    ...
    def test_personal_timetable_serialization(self):
        timetable = PersonalTimetable.objects.create(
            semester=self.sem, school="uoft", has_conflict=False, student=self.student
        )
        timetable.courses.add(self.course)
        timetable.sections.add(self.section)
...
class UrlsTest(UrlTestCase):
    ...
    def test_urls_call_correct_views(self):
        ...
        # timetable sharing
        self.assertUrlResolvesToView(
            "/timetables/links/", "timetable.views.TimetableLinkView"
        )
        self.assertUrlResolvesToView(
...
class TimetableViewTest(APITestCase):
    ...
    def test_create_timetable(self):
        ...
        response = self.client.post(
            "/timetables/", request_data, format="json", **self.request_headers
        )
