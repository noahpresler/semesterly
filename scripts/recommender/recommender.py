...
class Recommender:
    ...
    def train(self):
        ...
        print(
            "Training {0} on {1} features, {2} timetables".format(
        ...
        for c1 in bar(list(range(num_fts))):
            ...
            if len(similar) > 0 and (len(c1_rows) < 5 or len(similar) < 3):
                low_data_courses[c1] = []
            for c2 in similar:
                css = self.compute_similarity(
                    feat_trix[:, c1].toarray(), feat_trix[:, c2].toarray()
                )
    ...
    def predict(self, cid, similarities=None, force_print=False):
        ...
        ret = [
            x
            for x in sorted(similarities[cid], key=lambda x: x[1], reverse=True)
            if x[0] != cid
        ]
        fall = filter(
            lambda x: Course.objects.filter(
                id=x[0], section__semester__in=Semester.objects.filter(name="Fall")
            ).exists(),
            ret,
        )[:5]
        spring = filter(
            lambda x: Course.objects.filter(
                id=x[0], section__semester__in=Semester.objects.filter(name="Spring")
            ).exists(),
            ret,
        )[:5]
    ...
    def recommend(self, course_ids, similarities=None):
        ...
        recs = [
            x
            for x in sorted(list(recs.items()), key=lambda x: x[1], reverse=True)
...
def main():
    parser = argparse.ArgumentParser(
        description="Recommender system using collaborative filtering"
    )
    parser.add_argument(
        "--action",
        dest="action",
        required=True,
        choices=["train", "featurize", "predict", "save", "recommend"],
    )
    parser.add_argument(
        "--cids",
        dest="cids",
        default=None,
        type=str,
        help="List of course ids, comma seperated for recommending",
    )
    parser.add_argument(
        "--school", dest="school", required=True, help="School is required"
    )
    parser.add_argument(
        "--simfcn",
        dest="simfcn",
        default="cosine",
        type=str,
        choices=["pearson", "cosine", "hamming", "jaccard"],
    )
