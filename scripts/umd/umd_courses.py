...
class UMDParser(CourseParser):
    ...
    def find_url(self, div_class, parent):
        try:
            return parent.find(class_=div_class).get('href', '')
        except:
            return ''
    ...
    def get_departments(self):
        ...
        for link in prefix_a_tags:
            ...
            if self.semester is None or self.year is None:
                partial_url = department_url
            else:
                semester_month = semester_map[self.semester]
                partial_url = '{}{}/{}'.format(self.year,
                                               semester_month,
                                               department_url)
    ...
    def get_sections(self, section_url, course_model):
        ...
        for div in section_divs:
            ...
            for day in days:
                ...
                self.ingestor.ingest_offerings(section_model)
    ...
    def start(self,
        ...
        if departments:
            print("Error: Departments inputs is not supported.",
                  file=sys.stderr)
        if years is None:
            years = [self.year]
        if terms is None:
