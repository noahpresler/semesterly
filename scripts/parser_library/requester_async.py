...
class Requester:
    ...
    def http_async_request(self, do_http_request, *args, **kwargs):
        ...
        for i in range(10):
            try:
                with interruptingcow.timeout(timeout, exception=requests.exceptions.Timeout):
                    response = do_http_request().result()
    ...
    def get(self, url, params=None, async=False, **kwargs):
        ''' HTTP GET.
        ...
        if async:
            return self.http_async_request(request, 'GET', **kwargs)

        return self.http_request(request, 'GET', **kwargs)

    def post(self, url, form=None, params=None, data=None, async=False, **kwargs):
        ''' HTTP POST.
        ...
        if async:
            return self.http_async_request(request, 'POST', **kwargs)
