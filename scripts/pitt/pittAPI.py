...
class CourseAPI:
    ...
    def get_courses(self, term, subject):
        ...
        for course in courses:
            details = [
                course_detail.string.replace("&nbsp;", "").strip()
    ...
    def get_courses_by_req(self, term, req):
        ...
        url = "http://www.courses.as.pitt.edu/results-genedreqa.asp?REQ={}&TERM={}".format(
            req, term
        )
        page = urllib.request.urlopen(url)
        soup = BeautifulSoup(page.read())
    ...
    def get_class_description(class_number, term):
        ...
        for row in rows:
            ...
            for cell in cells:
                ...
                if (
                    len(cell.contents) > 0
                    and str(cell.contents[0]) == "<strong>Description</strong>"
                ):
