...
class OttawaParser:
    ...
    def get_search_results_html(self):
            ...
            try:
                selector = Select(
                    self.driver.find_element_by_id(
                        "ctl00_MainContentPlaceHolder_Basic_SessionDropDown"
                    )
                )
        ...
        self.driver.find_element_by_id(
            "ctl00_MainContentPlaceHolder_Basic_Button"
        ).click()
        while True:
            try:
    ...
    def parse_course_row(self, row):
        ...
        course_data = {"name": self.process_title(title), "description": description}
        course_obj, created = Course.objects.update_or_create(
            code=code, school="uo", defaults=course_data
        )
        ...
        for section_table in sections_tables:
            ...
            for row in rows:
                ...
                s_data = {"section_type": section_type, "instructors": prof}

                s_obj, _ = Section.objects.update_or_create(
                    course=course_obj,
    ...
    def parse_courses(self):
        ...
        while True:
            ...
            self.driver.execute_script(
                "__doPostBack('ctl00$MainContentPlaceHolder$ctl03','')"
            )
