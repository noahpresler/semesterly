...
class RutgersParser(BaseParser):
    def __init__(self, semester, campuses=["NB", "NK", "CM"], driver=None):
        BaseParser.__init__(self, RutgersCourse, RutgersCourseOffering, semester)
    ...
    def parse_course_element(self, course_element):
        ...
        description = (
            synopsis_element["onclick"].split('"')[1] if synopsis_element else ""
        )
        num_credits = course_element.find("span", class_="courseCredits").text.split()[
            0
        ]
        try:
    ...
    def parse_meeting_element(self, meeting_element):
        ...
        location = meeting_element.find(
            "span", class_="meetingTimeBuildingAndRoom"
        ).text
