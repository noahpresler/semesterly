...
parser.add_argument(
    "--school", dest="school", default=None, help="the school to send the message to"
)
parser.add_argument(
    "--message", dest="message", help="the school to send the message to"
)
parser.add_argument(
    "--title",
    dest="title",
    help="the school to send the message to",
    default="Semester.ly",
)
...
if not args.school:
    ...
    students = (
        Student.objects.filter(
            personaltimetable__courses__school=args.school
        ).distinct()
        | Student.objects.filter(school=args.school).distinct()
    )
...
for token in tokens:
    subscription_info = {
        "endpoint": "https://android.googleapis.com/gcm/send/fuKkKlzSEEE:APA91bFKiuJ3LEx7Ke3xOEJ…Lx-t8INikH97ewASUn6OSzRAGeGf8Eu1B5Q7Lju_7QBj5VeGjwCePUufhiSzFXqEogaJ4esAqA",
    ...
    headers.update(
        {"crypto-key": crypto_key, "content-encoding": "aesgcm", "encryption": salt}
    )
    fcm_data = {
