...
def start_JSON_populator(school, sem):
    ...
    file_path = os.path.join(
        module_dir, "../timetable/courses_json/" + school + "-" + sem + ".json"
    )
...
def convert_courses_to_json(courses, sem, school, limit=50):
    cs = []
    result_count = (
        0  # limiting the number of results one search query can provide to 50
    )
...
def has_offering(course, sem, school):
    ...
    try:
        res = SchoolCourseOffering.objects.filter(
            ~Q(time_start__iexact="TBA"),
...
def get_meeting_sections(course, semester, school):
    SchoolCourse, SchoolCourseOffering = school_to_models[school]
    offering_objs = SchoolCourseOffering.objects.filter(
        (Q(semester=semester) | Q(semester="Y")), course=course
    )
    sections = []
