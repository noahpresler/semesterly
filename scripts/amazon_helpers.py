...
def get_amazon_fields(isbn, api):
    try:
        result = api.item_lookup(
            isbn.strip(), IdType="ISBN", SearchIndex="Books", ResponseGroup="Large"
        )
        info = {
...
def get_all_sections(crs, semester):
    return list(
        Section.objects.filter((Q(semester__in=[semester, "Y"])), course=crs)
        .values_list("meeting_section", flat=True)
        .distinct()
    )
