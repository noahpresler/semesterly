...
export class FinalExamsModal extends React.Component {
    ...
    generateWeekHeaders(dates) {
        ...
        let html = dates.map((date, index) => {
            return <h3 key={date}><span
                className='day'>{($(window).width() > 766) ? days[index] : mobileDays[index]}</span><span
                className='date'>{date}</span></h3>
        })
            ...
            display =
                <div className="peer-card upsell">
                    <div className="peer-card-wrapper upsell cf">
                        ...
                        <p className="description">Add courses to find your final exams in a simple and intuitive
