    ...
    render() {
        ...
        if (this.props.fakeFriends) {
            ...
            for (let i = 0; i < this.props.fakeFriends; i++) {
                friendCircles[i] =
                    <div className="ms-friend" key={i} style={{backgroundImage: 'url(/static/img/blank.jpg)'}}></div>
            }
        } else {
            friendCircles = this.props.classmates && this.props.classmates.classmates ? this.props.classmates.classmates.map(c => {
                return <div className="ms-friend" key={c.img_url}
                            style={{backgroundImage: 'url(' + c.img_url + ')'}}></div>;
            }) : null;
        }
...
export const ShareLink = ({link, onClickOut}) => (
    <ClickOutHandler onClickOut={onClickOut}>
        <div id="share-course-link-wrapper">
            ...
            <input id="share-course-link" size={link.length} value={link} onClick={(e) => e.stopPropagation()}
                   readOnly/>
        </div>
    </ClickOutHandler>
)
