...
class CourseModalBody extends React.Component {
    ...
    return sections
      ...
      .map((section) => (
        <CourseModalSection
          ...
          lockOrUnlock={() =>
            this.props.addOrRemoveCourse(this.props.data.id, section.meeting_section)
          }
          hoverSection={() =>
            this.props.hoverSection({ course: this.props.data, section })
          }
  ...
  render() {
    ...
    if (sectionType != null) {
      const offeringSample =
        this.props.sectionTypeToSections[sectionType][0].offering_set[0];
            ...
            <div>
              <p>
                <p>
                  <img
                    alt="Short Course"
                    src="/static/img/short_course_icon_25x25.png"
                  />
      ...
      if (reaction) {
        return (
          <Reaction
            ...
            react={() => this.sendReact(cid, title)}
            emoji={title}
    ...
    const posTags =
      this.props.data.pos && this.props.data.pos.length ? (
    ...
    if (!this.props.isFetchingClassmates) {
      friendCircles =
        this.props.classmates.current.length > 0 ? (
      ...
      hasTakenCircles =
        this.props.classmates.past.length > 0 ? (
    ...
    if (!this.props.isLoggedIn || !this.props.hasSocial) {
      ...
      const conversionLink = !this.props.isLoggedIn ? (
        <a onClick={this.launchSignupModal}>
          <i className="fa fa-facebook" aria-hidden="true" />
          Link Facebook
        </a>
      ) : (
        <a onClick={this.enableSocial}>
          <i className="fa fa-facebook" aria-hidden="true" />
          Enable Facebook
        </a>
      );
    ...
    const avgRating =
      evalInfo.reduce((sum, e) => sum + parseFloat(e.score), 0) / evalInfo.length;
          ...
          <div className="attention__text">
            <span>
              ...
              <span className="highlight">
                {parseInt(this.props.popularityPercent, 10)}%
              </span>
    ...
    const capacityTracker = (
      <div className="capacity">
        <div className="capacity__tracker-text">
          <span>
            {parseInt(this.props.popularityPercent, 10)}% of Seats Added on Semesterly
          </span>
        </div>
      </div>
    );
      ...
      <div className="modal-body">
        <div className="cf">
          <div className="col-3-16">
            ...
            {showCapacityAttention && this.state.mobile && attentioncapacityTracker}
...
CourseModalBody.propTypes = {
  ...
  data: PropTypes.oneOfType([
    SemesterlyPropTypes.normalizedCourse,
    PropTypes.shape({}),
  ]),
