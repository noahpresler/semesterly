...
class PeerModal extends React.Component {
  ...
  render() {
    ...
    const optInClick = this.props.userInfo.isLoggedIn
      ? this.optInAll
      : this.optInSignUp;
    const upsell = (
      <div className="peer-card upsell">
    ...
    const height =
      this.props.peers[0] && this.props.peers[0].shared_courses
        ? this.props.peers[0].shared_courses.length * 30 + 95
        : 0;
    ...
    const ghostCards =
      !this.props.userInfo.social_all || peerCards.length === 0 ? (
        <div>
          {ghostCard}
          {ghostCard}
          {ghostCard}
