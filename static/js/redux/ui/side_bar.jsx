...
class SideBar extends React.Component {
    ...
    render() {
        ...
        if (optionalSlots.length === 0) {
            let img = (parseInt(masterSlots) != NaN && (masterSlots.length >= 4)) ? null :
                <img src="/static/img/emptystates/optionalslots.png"/>;
            optionalSlots = (
                <div className="empty-state">
                    { img }
                    <h4>Give Optional Courses a Spin!</h4>
                    <h3>Load this list with courses you aren't 100% sure you want to take - we'll fit as many as
...
export const TextbookList = ({courses}) => {
    ...
    let img = (parseInt(courses) != NaN && (courses.length >= 5)) ? null :
        <img src="/static/img/emptystates/textbooks.png"/>;
    if (tbs.length === 0) {
        return (<div className="empty-state">
            { img }
            <h4>Buy & Rent Textbooks: New, Used or eBook!</h4>
            <h3>Textbooks for your classes will appear here. Click to find the lowest prices, plus FREE two day shipping
