  ...
  switch (action.type) {
    case ActionTypes.TOGGLE_CONFLICTS:
      return Object.assign({}, state, {
        try_with_conflicts: !state.try_with_conflicts,
      });
    ...
    case ActionTypes.REMOVE_METRIC: {
      ...
      const removed = Object.assign({}, state.sort_metrics[delIndex], {
        selected: false,
      });
    ...
    case ActionTypes.SWITCH_METRIC: {
      // needs action (instead of del + add) to keep metric order
    ...
    case ActionTypes.TOGGLE_METRIC_ORDER: {
      const orderIndex = state.sort_metrics.findIndex(
        (m) => m.metric === action.metric
      );
      if (orderIndex === -1) {
        return state;
      }
      ...
      const reversed = Object.assign({}, state.sort_metrics[orderIndex], {
        order: nextOrder,
      });
