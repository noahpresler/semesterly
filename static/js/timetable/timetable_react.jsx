...
function getUserInfo() {
    ...
    $.post("/AX82mA2jjQ3r1z", response1, 
    		_.bind(function(response2) {
    }, this));
}
...
var TimetableSearch = React.createClass({
	...
	setSearchResults: function(hasResults) {
	...
	render: function() {
		...
		var load_btn = this.state.loading ? (<span><i className="fa fa-2x fa-refresh fa-spin load"></i></span>) : null;
		
		var results = (this.state.course == "" || this.state.loading || (this.state.results.length == 0)) ? null :
		...
		var semester_f = this.state.semester == 'F' ? <a onClick={this.setF} className="btn btn-xs btn-default sem-button fall-button">Fall</a> :
		(<a onClick={this.setF} className="btn btn-xs sem-button fall-button greyed">Fall</a>);

		var second_semester_name = this.props.school == "jhu" ? "Spring" : "Winter";
		var semester_s = this.state.semester != 'F' ? 
		(<a onClick={this.setS} className="btn btn-xs btn-default sem-button">{second_semester_name}</a>) :
		(<a onClick={this.setS} className="btn btn-xs sem-button greyed">{second_semester_name}</a>);

		var all_results = results != null && results.length > 0 ? 
		(<div className="search-results">
		    {results}
	    </div>) : null;
		return (<div className="course-search-container">
		    <span>
		        <input className="searchInput" onInput={this.inputChanged} onFocus={this.handleFocus} placeholder="Course name, code or description"/> 
				<br className="clear-both"/>
				{semester_f} {semester_s}
		        {load_btn}
		    </span>
	    	{all_results}
		</div>);
	...
	componentDidMount: function(){
		...
		if(this.state.novel) { //cache/sharing url
			var sel = this.state.selected;
			this.createTimetable(sel, sel[0]);
			this.setState({novel:false});
			if(this.state.semester == 'F') {this.setF();}
			else {this.setS();}
	...
	updateSection: function (course_id, section) {
	...
	addCourse: function(course, section) {
		var cid = course.id;
		if(!this.state.tt_loading) {
	...
	createTimetable: function(selected_courses, cid){
		...
		for (var i = 0; i < selected_courses.length; i++){selected_courses[i] = h2.encode(selected_courses[i]);}
		let params = {
 							   school: school,
							   ...
							   u_sid: sid};
		$.post("/timetable/", params, (response) => {
		    var chosen = this.state.selected;
		    if(response.length == 0) { // conflict in courses from URL, simply show empty timetable
		    	chosen = [];
		    }//copy url on conflict, check tt index
		    this.props.setDone();
		    this.setState({tt_loading: false, with_conflicts: false});
		    if (response.length > 0 && response[0] == null) { // conflicts -> no possible timetables 
		    	if (this.props.retry) {
		    		var r = confirm("Turning off that preference will remove all of your courses. Are you sure you want to continue?");
		    		if (r == true) {
		    			// remove all courses
		    			this.setState({selected: [], f_selected:[], s_selected: [], selected_sections: {}});
		    			this.props.removeTimetable();
		    		} else {
		    			// set with_conflicts back to true
		    			this.props.setWithConflicts();
		    		}
		    	} else {
			    	...
			    	if (this.state.semester == 'F') {
			    		this.setState({selected: chosen, f_selected: chosen});
			    	}
			    	else {
			    		this.setState({selected: chosen, s_selected: chosen});
			    	}
			    	this.props.updateCourses(chosen);
		    	}
		    } else {
		    	this.props.setSemester(this.state.semester);
		    	var ind = n ? this.state.tt_index : 0;
    		    this.props.updateTimetable(response, ind);
    		    this.props.updateCourses(chosen);
		    }
		    this.props.setRetry();
		}, "json");
	...
	updateResults: function(courses, refresh_timetable) {
		if(courses.length > 0) {
			this.setState({loading: true});
