...
var SearchResult = React.createClass({
    ...
    lockOrUnlockSection: function(new_section) {
    ...
    addCourseNoSection: function() {
        this.props.addCourse("")();
    },
    removeCourseNoSection: function() {
        this.props.removeCourse("")();
    },
    render: function() {
        var add_style = {width: '14px'};
        var action_icon = this.props.inRoster ? 
        <button type="button" className="btn btn-success in-roster" onClick={this.removeCourseNoSection}>
            <i className="fa fa-check" style={add_style}></i>
        </button>  :
        <button type="button" className="btn btn-info" onClick={this.addCourseNoSection}>
            <i className="fa fa-plus" style={add_style}></i>
        </button>;
       ...
       var course_display = 
            <div className="course-display">
                <div className="btn-group">
                    {action_icon} 
                    <button type="button" data-toggle="popover" style={buttonStyle}
        ...
        var sections_display = this.props.sections.map(function(section) {
            ...
            var locked_or_unlocked = this.props.inRoster && this.props.locked_section == section ? 
            (<span className="label label-default section-locked">
                <i className="fa fa-lock"></i> 
                {" " + section}
            </span>): 
            (<span className="label label-default section-unlocked">
                <i className="fa fa-unlock-alt"></i> 
                {" " + section}
            </span>);
            return (
                <a onClick={this.lockOrUnlockSection(section)}>
                    {locked_or_unlocked}
                    {br}
                </a>);
        }.bind(this));
