...
urlpatterns = patterns('',
                       ...
                       # api:
                       # url(r'^unsubscribe/(?P<id>[\w.@+-]+)/(?P<token>[\w.:\-_=]+)/$', student.views.unsubscribe), # patch?
                       # url(r'^user/save_settings/$', student.views.save_settings), # patch
                       # url(r'^me/*', timetable.views.profile),
                       # # single view:
                       # url(r'^user/save_timetable/$', student.views.save_timetable), # post
                       # url(r'^user/duplicate_timetable/$', student.views.duplicate_timetable), # post
                       # url(r'^user/delete_timetable/$', student.views.delete_timetable), # delete
                       # url(r'^user/get_saved_timetables/(?P<school>.+)/(?P<sem_name>.+)/(?P<year>[0-9]{4})',
                       #     student.views.get_student_tts_wrapper), # get
                       # # social
                       # url(r'^user/get_classmates/$', student.views.get_classmates), # get?
                       # url(r'^user/get_most_classmates_count/$', student.views.get_most_classmate_count), # get?
                       # url(r'^user/find_friends/*$', student.views.find_friends), # get?
                       # url(r'^user/add_to_gcal/*$', student.views.add_tt_to_gcal), # post
                       )
