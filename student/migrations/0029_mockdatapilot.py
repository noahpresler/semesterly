    ...
    if Semester.objects.filter(year="2020", name="Spring").exists():
        ...
        for code in pilot_codes:
            if Course.objects.filter(school="jhu", code=code).exists():
                ...
                courseint, created = CourseIntegration.objects.get_or_create(
                    course_id=course.id, integration_id=integration.id
                )
                courseint.semester.add(s20)
                courseint.save()
                if Section.objects.filter(course_id=course.id, semester=s20).exists():
                    sections = list(
                        Section.objects.filter(course_id=course.id, semester=s20)
                    )
                    for section in sections:
                        offering1, created = PilotOffering.objects.get_or_create(
                            day="M",
                            time_start="5:00pm",
                            time_end="7:00pm",
                            size=10,
                        ...
                        offering1.save()
                        offering2, created = PilotOffering.objects.get_or_create(
                            day="T",
                            time_start="5:00pm",
                            time_end="7:00pm",
                            size=10,
                        ...
                        offering2.save()
                        offering3, created = PilotOffering.objects.get_or_create(
                            day="W",
                            time_start="5:00pm",
                            time_end="7:00pm",
                            size=10,
...
class Migration(migrations.Migration):
    ...
    operations = [migrations.RunPython(add_mock)]
