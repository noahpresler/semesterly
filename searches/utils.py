...
def baseline_search(school, query, semester):
    """Baseline search returns courses that are contained in the name from a query
    (legacy code)."""
...
class Vectorizer:
    """Vectorizer class creates a dictionary over courses and build course vectors using
    count vectorizer."""
    ...
    def vectorize(self):
        """Vectorize function transforms and saves entire course objects into course
...
class Searcher:
    """Searcher class implements baseline search and vectorized search based on
    ...
    def get_similarity(self, query, course):
        """Vectorizes query and returns a cosine similarity score between query and
        course vector."""
    ...
    def vectorized_search(self, school, query, semester):
        ...
        if base_count < self.MAX_CAPACITY:
            descp_contains_query = reduce(
                operator.or_,
                list(map(course_desc_contains_token, query.replace("and", "").split())),
            )
            descp_matching_courses = Course.objects.filter(
                Q(school=school) & descp_contains_query & Q(section__semester=semester)
            ).exclude(
                reduce(
                    operator.and_, list(map(course_name_contains_token, query_tokens))
                )
            ...
            courses_objs = list(
                title_matching_courses.all().distinct("code")[: self.MAX_CAPACITY]
            ) + list(
                descp_matching_courses.all().distinct("code")[
                    : self.MAX_CAPACITY - base_count
                ]
            )
        else:
            courses_objs = list(
    ...
    def get_most_relevant_filtered_courses(self, query, course_filtered):
        """Returns the most relevant filtered courses given a query from filtered course
        objects."""

        query_vector = self.vectorize_query(query.lower())
        return sorted(
            course_filtered,
            key=lambda course: -self.get_score(course, query, query_vector),
        )

    def get_score(self, course, query, query_vector):
        """Computes similarity score based on cosine similarity and match between query
        and course name."""

        return self.get_cosine_sim(query_vector, course.vector) + self.matches_name(
            query, course.name
        )
    ...
    def print_similiarity_scores(self, courses, query):
        ...
        for course in sorted(
            courses, key=lambda course: -self.get_score(course, query, query_vector)
        )[:10]:
            print(
                (
                    course.name
                    + ":"
                    + str(self.get_score(course, query, query_vector))
                    + "\n"
                )
            )
