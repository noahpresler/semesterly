...
class UrlsTest(UrlTestCase):
    ...
    def test_urls_call_correct_views(self):
        self.assertUrlResolvesToView(
            "/integrations/1234/course/5678/",
            "integrations.views.IntegrationsView",
            kwargs={"integration_id": "1234", "course_id": "5678"},
        )


class IntegrationsGetAddTest(APITestCase):
    request_headers = {"HTTP_HOST": "uoft.sem.ly:8000"}
    ...
    def setUp(self):
        ...
        self.courseIntegration = CourseIntegration.objects.create(
    ...
    def test_get_nonexistent_integration(self):
        ...
        response = self.client.get(
            "/integrations/"
            + nonexistentIntegrationIdStr
            + "/course/"
            + nonexistentCourseIdStr
            + "/",
            format="json",
            **self.request_headers
        )
    ...
    def test_add_integration(self):
        ...
        CourseIntegration.objects.get(
            course_id=self.course.id, integration_id=self.integration.id, **data
        )
    ...
    def test_delete_integration(self):
        ...
        self.assertFalse(
            CourseIntegration.objects.filter(
                course_id=self.course.id, integration_id=self.integration.id
            ).exists()
        )
